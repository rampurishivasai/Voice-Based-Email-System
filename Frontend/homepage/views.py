...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        addr='miniprojectbatch19@gmail.com'
        print(addr)
        request.email = addr
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        for item in toaddr:
            ...
            # item = "rampurishivasai@gmail.com"
            newtoaddr.append(item)
            print(item)

        msg = MIMEMultipart()
        msg['From'] = fromaddr
        # newtoaddr = ["rampurishivasai@gmail.com"]
        ...
        try:
            ...
            texttospeech("Your email has been sent successfully.", file + i)
            i = i + str(1)
            return JsonResponse({'result': 'failure'})
        s.quit()
        return JsonResponse({'result' : 'success'})
